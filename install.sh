...
# base
sudo apt-get install zsh curl gawk gcc git gzip less make tar tmux trash-cli unzip vim wget whois zip tree ripgrep
# extend
sudo apt-get install bzip2 dos2unix htop httping iftop iotop nmap openssl rsync sshfs sshpass snap
# docker
sudo apt-get install docker-compose linux-image-extra-virtual  linux-modules-extra-$(uname -r)
# progress(cp mv dd ...进度显示)
sudo apt-get install libncurses5-dev libappindicator1 libindicator7 progress
# qemu
