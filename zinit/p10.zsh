# Generated by Powerlevel10k configuration wizard on 2020-07-23 at 10:15 CST.
...
() {
  ...
  function my_git_formatter() {
    ...
    if (( $1 )); then
      ...
      local      clean='%248F'   # green foreground
      local    commits='%6F'  # cyan foreground
      local   modified='%248F'  # yellow foreground
      local  untracked='%39F'   # blue foreground
      local conflicted='%196F'  # red foreground
    else
      # Styling for incomplete and stale Git status.
      local       meta='%240F'  # grey foreground
      local      clean='%240F'  # grey foreground
      local    commits='%240F'  # grey foreground
      local   modified='%240F'  # grey foreground
      local  untracked='%240F'  # grey foreground
      local conflicted='%240F'  # grey foreground
    fi
    ...
    (( VCS_STATUS_COMMITS_BEHIND )) && res+=" ${commits}⇣${VCS_STATUS_COMMITS_BEHIND}"
    # ⇡42 if ahead of the remote; no leading space if also behind the remote: ⇣42⇡42.
    (( VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_COMMITS_BEHIND )) && res+=" "
    (( VCS_STATUS_COMMITS_AHEAD  )) && res+="${commits}⇡${VCS_STATUS_COMMITS_AHEAD}"
    # ⇠42 if behind the push remote.
    (( VCS_STATUS_PUSH_COMMITS_BEHIND )) && res+=" ${commits}⇠${VCS_STATUS_PUSH_COMMITS_BEHIND}"
    (( VCS_STATUS_PUSH_COMMITS_AHEAD && !VCS_STATUS_PUSH_COMMITS_BEHIND )) && res+=" "
    # ⇢42 if ahead of the push remote; no leading space if also behind: ⇠42⇢42.
    (( VCS_STATUS_PUSH_COMMITS_AHEAD  )) && res+="${commits}⇢${VCS_STATUS_PUSH_COMMITS_AHEAD}"
  ...
  typeset -g POWERLEVEL9K_CONTEXT_{REMOTE,REMOTE_SUDO}_FOREGROUND=$grey
